...
theta_opt = logistic_regression_gd(X_train, y_train, learning_rate,n_iters);

% Print out the classification error percentage
e = classif_error(y_val,log_regr(X_val,theta_opt))


% For testing the model with different training dataset sizes (last part of Task 3), I reused
% very similar code to that of Task 1. I removed it from this section for
% clarity as it did not really add anything new. The important results are
% in my report.
...
function theta_opt = logistic_regression_gd(X_train, y_train, learning_rate, iters_total)
    % Implements logistic regression using GD
